...
import { toggleUserStatus, getUsersRecord, changeUserStatus, getUserDetails } from 'store/reducers/users';
export default function UserComfirmModal({
  modalFlagConfirm,
  ...
  return (
    <Dialog
      open={modalFlagConfirm?.userConfirmModal}
      onClose={() => {
        // handleToggle();
        updateModalFlagConfirm('userConfirmModal', false);
      }}
          ...
          <Button
            ...
            onClick={async () => {
              if (userDetails?.is_active === 1) {
                setSelectedStatus(0);
                const res = await dispatch(toggleUserStatus({ id, is_active: false }));
                updateModalFlagConfirm('userConfirmModal', false);
                getUsersRecord(count);
              } else {
                setSelectedStatus(1);
                const res = await dispatch(toggleUserStatus({ id, is_active: true }));
                getUsersRecord(count);
                updateModalFlagConfirm('userConfirmModal', false);
              }
            }}

...
// assets
import { CaretUpOutlined, CaretDownOutlined, DragOutlined, CheckOutlined, DownloadOutlined, DeleteOutlined } from '@ant-design/icons';
    ...
    <Grid container alignItems="center" justifyContent="flex-end" sx={{ width: 'auto' }}>
      ...
      <Grid item sx={{ mt: { xs: 2, sm: 0 } }}>
        <TablePaginationMUI
          ...
          page={pageIndex}
          //@ts-ignore
          onPageChange={(event, newPage) => handleChangePagination(event, newPage + 1)}
          rowsPerPage={pageSize}
          onRowsPerPageChange={(event) => handleChange(event)}
...
export const TableRowSelection = ({ selected, onDelete }: { selected: number; onDelete?: () => void }) => (
  <>
    {selected > 0 && (
      <Stack direction="row" alignItems="center" spacing={1} sx={{ justifyContent: 'flex-end', padding: '0 24px' }}>
        <IconButton sx={{ fontSize: '24px', color: 'red' }}>
